...
def load_data():
    ...
    df_agg = pd.read_csv('data/Aggregated_Metrics_By_Video.xls').iloc[1:,:] # does not import first row
...
# create dataframes from load_data function 
...
# df_agg_diff = (df_agg_diff.select_dtypes(include=np.number) - median_agg).div(median_agg) # * THIS REMOVES NON-NUERIC COLUMNS *

# create differences from the median value; just for numeric columns (but retain all non-numeric columns in the df)
...
if add_sidebar == 'Aggregate Metrics':
    st.write('KJ Youtube channel: Aggregate metrics')
    ...
    col1, col2, col3, col4, col5 = st.columns(5)    # set up columns to display metrics
    ...
    for i in metric_medians6mo.index:
        with columns[count]:
            ...
            st.metric(label= i, value = round(metric_medians6mo[i],1), delta = "{:.2%}".format(delta))  # insert metrics on dashboard
            count += 1
            if count >= 5:
                count = 0

    # get date information / trim to relevant data (for dataframe display on dashboard)
...
if add_sidebar == 'Individual Video Analysis':
    ...
    # order axis 
    st.plotly_chart(fig)    # add chart to st dashboard
    ...
    st.plotly_chart(fig2)   # add chart to st dashboard